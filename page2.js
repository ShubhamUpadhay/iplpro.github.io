...
$(document).ready(function () {
    ...
    for (let i = 0; i < tarr.length; i++) {
        ...
        console.log(tarr[i].team)
        if (DataNeeded == tarr[i].team) {
            $("#teamname").text(tarr[i].team)
            let card = $('<div>').append(
                $('<div>').append($('<img>').attr("src", tarr[i].photo)).addClass("imgC"),
                $('<div>').append(
                    $('<h4>').text(tarr[i].name),
                    $('<h5>').text("Price: " + tarr[i].price),
                    $('<h5>').text("Status " + tarr[i].playingStatus),
                    $('<h5>').text("Role: " + tarr[i].Role),
                ).addClass("info")
            );
    ...
    if (DataNeeded == "Chennai Super Kings") {
        $('.imgC').css({ "background": "linear-gradient(70deg, yellow,orange" })
        $('.info').css({ "background": "linear-gradient(70deg, yellow,orange" })
        $("main").css({ "background": "linear-gradient(70deg, goldenrod,#eed202)" })
    }
    if (DataNeeded == "Delhi Capitals") {
        ...
        $("main").css({ "background": "linear-gradient(70deg,#000080, 70% ,blue)" })
    } if (DataNeeded == "Kolkata Knight Riders") {
        ...
        $("main").css({ "background": "linear-gradient(70deg,#800080, 70%,goldenrod)" })
    } if (DataNeeded == "Mumbai Indians") {
        ...
        $("main").css({ "background": "linear-gradient(lightgrey, 70%,silver)" })
    } if (DataNeeded == "Punjab Kings") {
        ...
        $("main").css({ "background": "linear-gradient(70deg, rgb(247, 30, 30),70%,pink)" })
    } if (DataNeeded == "Rajasthan Royals") {
        ...
        $("main").css({ "background": "linear-gradient(70deg,blue, 30%,#e75480)" })
    } if (DataNeeded == "Royal Challengers Bangalore") {
        $('.imgC').css({ "background": "linear-gradient(70deg, red, 60%,black)" })
        $('.info').css({ "background": "linear-gradient(70deg, red, 60%,black)" })
        $("main").css({ "background": "linear-gradient(70deg, darkred, 70%,black)" })
    } if (DataNeeded == "Sunrisers Hyderabad") {
        $('.imgC').css({ "background": "linear-gradient(70deg, #fc8a00, 60%,grey)" })
        $('.info').css({ "background": "linear-gradient(70deg, #fc8a00, 60%,grey)" })
        $("main").css({ "background": "linear-gradient(70deg, #fc8c00, 70%,grey)" })
    }
})
